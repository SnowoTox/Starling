...
namespace Starling.PS3MAPI
{
    public class PS3MAPI
    {
       ...
       /// <summary>
       /// Connects to the specified target with a WinForm. This function does not allow the usage of public IP Addresses.
       /// </summary>
       /// <returns></returns>
        public bool ConnectTarget()
        {
            ...
            frm.Text = "PS3MAPI Connect";
            frm.MinimizeBox = false;
            frm.MaximizeBox = false;
            frm.BackColor = System.Drawing.Color.Black;
            frm.MaximumSize = new System.Drawing.Size(300, 200);
            frm.MinimumSize = new System.Drawing.Size(300, 200);

            TextBox ipAddr = new TextBox();
            ipAddr.Location = new System.Drawing.Point(5, 10);
            ipAddr.Width = 275;
            ipAddr.Text = "IP Address ";
            ipAddr.BackColor = System.Drawing.Color.Black;
            ipAddr.ForeColor = System.Drawing.Color.Cyan;

            TextBox port = new TextBox();
            port.BackColor = System.Drawing.Color.Black;
            port.ForeColor = System.Drawing.Color.Cyan;
            port.MaxLength = 5;
            port.Text = "PORT 7887";
            port.Width = 275;
            port.Location = new System.Drawing.Point(5, 40);

            Button OK = new Button();
            OK.Text = "Connect To Target";
            OK.BackColor = System.Drawing.Color.Black;
            OK.ForeColor = System.Drawing.Color.Cyan;
            OK.Width = 275;
            OK.Location = new System.Drawing.Point(5, 70);

            frm.Controls.Add(ipAddr);
            frm.Controls.Add(port);
            frm.Controls.Add(OK);

            OK.Click += (object sm, EventArgs de) =>
            {
                if (string.IsNullOrEmpty(ipAddr.Text) || string.IsNullOrWhiteSpace(ipAddr.Text) || string.IsNullOrEmpty(port.Text) || string.IsNullOrWhiteSpace(port.Text))
                {
                    result = false;
                ...
                {
                    // This variable won't be used. This is just a placeholder for the check.
                    IPAddress placeholder;

                    if (Regex.IsMatch(ipAddr.Text, @"^[a-zA-Z]$"))
                    {
                        //Replaces any letter with an empty value.
                        Regex.Replace(ipAddr.Text, @"^[a-zA-Z]$", "");
                    }

                    // Checks if any letter is present in port Textbox. 
                    if (Regex.IsMatch(port.Text, @"^[a-zA-Z]$"))
                    {
                        //Replaces any letter with an empty value.
                        Regex.Replace(port.Text, @"^[a-zA-Z]$", "");
                    }

                    // Checks if the specified IP Address is a valid IP Address.
                    if (IPAddress.TryParse(ipAddr.Text, out placeholder))
                    {
                        // erases the value of the placeholder to prevent wasting of usage from this DLL.
                        placeholder = null;

                        // goes to the next if statement, if the IP Address is not a local 192 address.
                        if(!ipAddr.Text.StartsWith("192."))
                        {
                            // goes to the next statement, if the IP Address is not a local 172 address.
                            if(!ipAddr.Text.StartsWith("172."))
                            {
                                // .
                                if(!ipAddr.Text.StartsWith("10."))
                                {
                                    MessageBox.Show("Public IP Addresses are not allowed due to safety reasons.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    result = false;
                                }
                                else
                                {
                                    // Converts the string numbers to an integer.
                                    int portNEW = Convert.ToInt32(port.Text);

                                    // Connects to PS3MAPI.
                                    PS3MAPI_Client_Server.Connect(ipAddr.Text, portNEW);
                                    if (IsConnected)
                                    {
                                        result = true;
                                        frm.Close();
        ...
        public bool AttachProcess(uint pid)
        {
            ...
            uint[] getprocs = Process.GetPidProcesses();

            if (getprocs.Length > 0)
            {
                for (int i = 0; i < getprocs.Length; i++)
                {
                    if (getprocs[i] == pid)
                    {
        ...
        public bool AttachProcess()
        {
            ...
            if (getprocs.Length > 0)
            {
                ulong uProc = getprocs[0];
                uint Procid = Convert.ToUInt32(uProc);

                Process.Process_Pid = Procid;
                result = true;
            }
            else result = false;
        ...
        /// <summary>Writes the log in this string.</summary>
